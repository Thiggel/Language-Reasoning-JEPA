...
is the obvious fix. (ii) A model with \textbf{no value head} still plans at
...
\begin{itemize}\itemsep2pt
  \item \textbf{Decouple the two energies}: straighten a learned
        \emph{projection} $\pi(s)$ instead of $s$ itself, so goal-distance
        planning and content decodability stop competing (Result 8).
