...
\begin{frame}{Scale-up preview --- the hard domain and faithful iGSM}
\claim{On 10--18-step problems everything collapses --- the likelihood
selector hardest --- and the ranking/capacity/search recipe degrades most
gracefully. This is the quantified frontier for the next phase.}
\centering\small
\begin{tabular}{lcccc}
\toprule
...
minimal-labels + cost rank (\texttt{mdr\_cal}, 9M) & \textbf{0.120} &
...
look-4). Separately, the pipeline now runs on \textbf{100\%-faithful iGSM}
(the official facebookresearch generator vendored; our env drives the
reference renderer; full solutions pass the official checker 60/60 med,
99/100 hard) --- first faithful-domain training runs completed, planning
