...
\texttt{core5} = \textbf{the sheared recipe} (latent pred + ranking + VICReg + anchor + LDAD); \texttt{*\_cal} = + cost ranking; \texttt{*\_s2/s3} = seed replicates.
\end{frame}

\begin{frame}{THE recipe (sheared) and headline results (mean $\pm$ std, 3 seeds)}
\claim{Five losses suffice --- everything else adds $\le$1--3\% and is
reported in the shear log.}
\centering\small
\begin{tabular}{lcc}
\toprule
 & @optimal & @slack-2\\
...
\textbf{core recipe (\texttt{core5})} & \textbf{0.880 $\pm$ 0.004}
 & \textbf{0.975 $\pm$ 0.004}\\
\quad + depth option (full stack, look-2) & 0.945 & 1.00\\
symbolic oracle & 1.00 & 1.00\\
\bottomrule
\end{tabular}

\medskip
\textbf{core5} = latent prediction + counterfactual ranking + VICReg +
frozen-anchor prediction + LDAD; the value head is trained by ranking
margins alone --- \emph{no numeric value labels}. The depth option
(cost ranking, look-2) needs the fuller stack (Results 12, 16); on the
sheared recipe it does not pay (0.885 look-2 vs 0.900 look-1).
\end{frame}
...
\begin{frame}{Result 16 --- the shear log (how the recipe was found)}
\claim{Build up from nothing; shear down from everything: five components
carry the result, each further addition is $\le$1--3\% ($\approx$ seed noise).}
\centering\small
\begin{tabular}{lccc|lc}
\toprule
\textbf{build-up} & losses & @opt & eff.\ rank &
 \textbf{sheared (from full)} & $\Delta$@opt\\
\midrule
ranking only & 1 & 0.20 & \textbf{24 (collapse)} & hinge & $-$0.02\\
+ latent prediction & 2 & 0.78 & 101 & distilled $V$ & $-$0.04\\
+ VICReg & 3 & 0.745 & 234 (healthy) & rollout supervision & $\sim$0\\
+ anchor + LDAD = \textbf{core5} & 5 & \textbf{0.88$\pm$.004} & 234 &
...
\medskip\small
Each sheared component retains a specific \emph{auxiliary} role: hinge +
distilled $V$ = the fully label-free-energy variant (0.36--0.57);
EMA = $+$0.03 over stopgrad+VICReg (which suffice for stability ---
\texttt{noema} 0.605, no collapse); cost ranking = depth calibration, but
only on the fuller stack (on core5: 0.885 look-2 $<$ 0.900 look-1; on the
full stack: 0.945). \textbf{Failures remain chain-depth-limited}: 4.4\%
(3--4 steps) $\to$ 59\% (7--9). Edit track mirrors the shear:
\texttt{edit\_core5} 0.455/0.485 $=$ champion, no defect labels.
\end{frame}
