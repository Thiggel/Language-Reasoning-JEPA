...
\subtitle{Method and first two experimental cycles --- fully reproducible from these slides}
...
\smallskip
Run names: \texttt{disc\_*} = this (discourse) track;
...
\texttt{disc\_chunkpred}/\texttt{combo} (\textbf{frozen} anchor
$\tilde E_c$). Value head input detached except
\texttt{disc\_valgrad}/\texttt{combo}. $d^{\text{goal}}_t$ = LN-L1 distance
...
\midrule
\texttt{disc\_straight\_\{lo,mid,---\}} & frozen, w=2.0 & \textbf{yes}
  & $+\,\mathcal L_{\text{curv}}$, $\lambda\in\{.02,.05,.1\}$\\
\texttt{disc\_mono\,/\,\_hi} & frozen, w=2.0 & \textbf{yes}
  & $+\,\mathcal L_{\text{mono}}$, w1\,m.02\,/\,w2\,m.05\\
\texttt{disc\_geo\_lo\,/\,\_geo} & frozen, w=2.0 & \textbf{yes} & both regularizers\\
\texttt{disc\_mono\_novalue} & frozen, w=2.0 & no
  & $+\,\mathcal L_{\text{mono}}$, \emph{no value head}\\
\midrule
\texttt{disc\_rank\_\{k2,k4\}(\_nodelta)} & frozen, w=2.0 & \textbf{yes}
  & $+$ ranking over K counterfactual actions\\
\texttt{disc\_champion} & frozen, w=2.0 & \textbf{yes}
  & ranking K=2 $+$ $\mathcal L_{\text{mono}}$ w2\\
\texttt{disc\_\{shuffled, no\_distract, size*\}} & frozen, w=2.0 & \textbf{yes}
...
With slack 2: base 0.75, +anchor 0.88, +value-grad 0.92,
+monotonicity \textbf{0.935} (random 0.41).\\[4pt]
...
imagined end state; no retraining.\\[4pt]
...
\midrule
\texttt{disc\_combo} ($\lambda=0$) & 0.15 & 0.52 & 0.635 & 0.89\\
\texttt{disc\_straight\_mid} ($\lambda{=}.05$) & 0.365 & \textbf{0.845} & 0.355 & 0.73\\
\texttt{disc\_mono\_hi} (hinge w2 m.05) & 0.255 & 0.68 & \textbf{0.655} & \textbf{0.935}\\
\texttt{disc\_mono\_novalue} (\emph{no} $V$) & 0.20 & 0.65 & --- & ---\\
\texttt{disc\_value\_only} (\emph{no} JEPA) & 0.06 & --- & 0.075 & 0.43\\
...
0.65 @slack 2 from pure geometry. (iii) \texttt{disc\_value\_only}'s
...
\midrule
\texttt{disc\_combo} & 0.635 & 0.89 & 0.76 & 0.73\\
\texttt{disc\_rank\_k2} & 0.910 & 0.985 & 0.85 $\downarrow$ & 0.94\\
\texttt{disc\_rank\_k4} & 0.925 & 0.98 & 0.85 $\downarrow$ & 0.94\\
\texttt{disc\_rank\_k4} \emph{no LDAD} & 0.875 & 0.965 & --- & 0.94\\
\texttt{disc\_combo} \emph{no LDAD} & 0.48 & 0.805 & --- & 0.69\\
\textbf{\texttt{disc\_champion}} (rank+mono) & \textbf{0.905} & \textbf{0.99}
 & --- & \textbf{0.96}\\
\bottomrule
\end{tabular}
...
cost sums need. \texttt{disc\_champion} also has the best raw geometry
...
\midrule
\texttt{disc\_rank\_k2} & 0.910 & 0.850 $\downarrow$ & 0.915 & \textbf{0.970}\\
\texttt{disc\_rank\_cal} (+CostRanking) & 0.905 & \textbf{0.945} & --- & 0.915\\
\texttt{disc\_champion\_cal} & 0.910 & 0.945 & --- & 0.915\\
\bottomrule
\end{tabular}
...
\claim{Shape the metric with the monotonicity hinge, distill it into $V$,
add binary-order ranking: \textbf{0.935 @optimal / 0.99 @slack-2} with no
scalar value labels anywhere — beating the scalar-label champion.}
\centering\small
\begin{tabular}{llcc}
\toprule
run & energy supervision & @opt & @slack2\\
\midrule
\texttt{disc\_champion\_cal} & scalar + ranking & 0.910 & 0.995\\
\textbf{\texttt{disc\_mono\_distill\_rank}} & \textbf{binary only}
 & \textbf{0.935} & 0.990\\
\texttt{disc\_mono\_hi} & scalar regression & 0.655 & 0.935\\
\texttt{disc\_mono\_distill} & binary (no ranking) & 0.530 & 0.885\\
\texttt{disc\_mono\_lf\_distill} & \emph{none} (all-steps hinge) & 0.355 & 0.775\\
\texttt{disc\_geometric} & \emph{none} (curvature) & 0.300 & 0.715\\
\texttt{disc\_distill\_v} & none, \emph{unshaped} (control) & 0.190 & 0.640\\
\bottomrule
\end{tabular}

\medskip\small
$V$ is distilled from the latent goal distance $d^{\text{goal}}$ — the
energy \emph{emerges from the JEPA geometry}; labels only shape the metric
(binary necessary/distractor for the hinge, binary order for ranking).
Shaping is required: unshaped distillation fails (0.19). Seed replicate:
0.560/0.880 (mono\_distill\_s2).
\end{frame}
...
\midrule
\texttt{disc\_ldad\_pure} & --- & --- & --- & \textbf{collapse}
 (std .027, rank 80)\\
\texttt{disc\_ldad\_pure\_sg} & --- & \checkmark & --- & collapse (0.11 @opt)\\
\texttt{disc\_combo\_novic} & \checkmark & \checkmark & --- & 0.27 @opt
 (vs 0.635)\\
\texttt{disc\_combo} & \checkmark & \checkmark & \checkmark & 0.635 @opt\\
\bottomrule
\end{tabular}
...
\texttt{disc\_base} manages only 0.33 even then (collusion).\\[4pt]
...
\begin{itemize}\itemsep2pt
  ...
  \item \textbf{Error analysis of the last 10\%}: what do the
        \texttt{disc\_champion} failures share (chain depth, op mix,
        tie patterns)?
