...
for length_mode in oracle fixed_budget; do
  "$python_bin" "${TEXTJEPA_ROOT}/scripts/eval_pooled_sentence_planning.py" \
    --ckpt "$ckpt" --device cuda:0 --examples 32 --max-tokens 64 \
...
"$python_bin" "${TEXTJEPA_ROOT}/scripts/probe_pooled_sentence_states.py" \
  --ckpt "$ckpt" --device cuda:0 --train-examples 512 --test-examples 256 \
...
"$python_bin" "${TEXTJEPA_ROOT}/scripts/train_detached_sentence_decoder.py" \
  --ckpt "$ckpt" --device cuda:0 --train-examples 1024 --test-examples 256 \
  --batch-size 64 --epochs 5 --output "$model_dir/detached_sentence_decoder.json"
