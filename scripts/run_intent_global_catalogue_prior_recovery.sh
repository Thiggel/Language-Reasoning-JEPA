...
"$python_bin" "${TEXTJEPA_ROOT}/scripts/train.py" \
  ...
  "run_name=global_catalogue_prior_lr${learning_rate}" \
  "train.init_ckpt=$checkpoint" \
  "train.lr=$learning_rate" "train.epochs=$epochs" \
  "data.train_size=$train_size" data.val_size=1000 \
  "device=${DEVICE:-cuda:0}"

# The preceding pilot ruled out availability-weight calibration as the main
# failure. Keep that weight fixed and spend evaluation compute on the causal
# depth comparison.
for depth in 1 2 4; do
  for scorer in prior jepa; do
    ...
    "$python_bin" "${TEXTJEPA_ROOT}/scripts/eval_intent_generalization.py" \
      ...
      --proposal-support-weight 1.0 \
      "${extra[@]}" \
      --out "$model_dir/catalogue_support1.0_depth${depth}_${scorer}.json"
  done
done
