...
common=("data=$dataset" "seed=$seed" "device=$device" "train.lr=$learning_rate"
  "train.epochs=$epochs" "train.batch_size=$batch_size" "train.num_workers=2"
...
"$python_bin" - "$RUN_DIR/training_complete.json" "$dataset" "$family" "$epochs" "$train_size" "$batch_size" "$learning_rate" <<'PY'
import json, os, pathlib, sys
p = pathlib.Path(sys.argv[1]); tmp = p.with_suffix('.tmp')
tmp.write_text(json.dumps({'dataset':sys.argv[2], 'family':sys.argv[3], 'epochs':int(sys.argv[4]), 'train_size':int(sys.argv[5]), 'batch_size':int(sys.argv[6]), 'learning_rate':float(sys.argv[7]), 'status':'completed'}, indent=2)+'\n')
os.replace(tmp, p)
PY
