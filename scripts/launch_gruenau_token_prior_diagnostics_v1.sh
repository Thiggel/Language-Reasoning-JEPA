...
NAMES=(
  ...
  mlp64_w1 mlp256_w05 mlp256_w1 detach_w05 detach_w1 smooth01_w1
  nophase_w1 dense1_w1 dense4_w1 l1_w1 span4_16_w1 span8_64_w1
)
...
for index in "${!NAMES[@]}"; do
  ...
  printf -v command '%q ' env CUDA_VISIBLE_DEVICES="$gpu" \
    TEXTJEPA_ROOT="$ROOT" "$ROOT/scripts/run_token_prior_diagnostics_v1.sh" \
    "$name" 0
  remote="cd $(printf %q "$ROOT"); nohup $command > $(printf %q "$ROOT/runs/overnight_token_prior_logs/${name}_diagnostics_v1.log") 2>&1 < /dev/null & echo \$!"
