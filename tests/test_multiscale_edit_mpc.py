...
def test_content_only_prior_enumerates_positions_by_token_confidence():
    vocab = _vocab()
    ...
    mask = vocab.token_to_id["<mask>"]
    state = planner.encode(
        [[vocab.token_to_id["prompt"]]], [[mask, mask]]
    )
    proposed = planner.candidates(
        state, [[vocab.token_to_id["prompt"]]], [[mask, mask]], False
    )
    assert len(proposed) == 1
    action, score = proposed[0]
