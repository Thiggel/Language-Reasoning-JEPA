...
## 2026-08-25 (night-2) — culprit ladder on scr-base launched; rung-0 decomposition: the depth cliff is WANDERING, not failure-to-reach
- scr-base/rollpred/solprob (from-scratch canonical) finals in: base d2 .96/.86, d4 .89/.48, d8 .80/.44 (success/answer). rollpred+solprob DO NOT beat base from scratch — their earlier gains likely compensated legacy render noise. scr-causal(+rollpred) both CUDA-OOM'd ~2h in (fp32 doesn't fit at this config even with expandable_segments; needs smaller batch or checkpointing) — transformer-predictor question still open.
- Rung 0 (re-read of scr-base episodes): solved barely drops with depth (.96/.89/.80) but answer|solved collapses .90/.54/.55 and overshoot doubles (steps/necessary 1.29/1.92/2.16, exact|solved .62/.15/.11). The planner reaches the goal at depth but MEANDERS; long noisy trajectories then break answer emission. The culprit question becomes: what makes deep search pick unnecessary-but-feasible actions?
- Launched paired eval-only 2x2 ladder on scr-base best.pt (Grünau, round 2026-08-25-scr-base-culprit-v1, canonical, 100 eps, same seed/split as finals): lane A true-states+LN-L1 ruler d1/2/4/8; lane B true-states+energy head d2/4/8; lane C imagined+oracle-goal LN-L1 d2/4/8 (+d1 energy control); lane D canonical noise/drift probe + prior_propose coverage (energy & symbolic-oracle); lane E symbolic-oracle-through-search control d4/d8 (harness certification on this ckpt). Read-out: whichever oracle swap restores exact|solved and kills overshoot is the culprit; manifold kept consistent per arm (all-encoder vs all-predicted), ln_l1 everywhere (raw-L2 bug avoided).

## 2026-08-26 — bf16 "NaN" root-caused: Blackwell CUDA illegal-memory-access at batch 16, NOT a numerics bug
- Isolation probes (2k problems, 60 steps, rtxpro6k): bf16 batch8 clean (with AND without latent_rollout_pred, all SDPA backends, all objectives healthy); bf16 batch16 → cudaErrorIllegalAddress in backward at step 0 (with or without rollpred). Yesterday's "nan from step 0" was memory corruption from this kernel bug (its step 0 was actually finite; nan by step 20). A6000 (Ampere) bf16 fully clean at any batch → Blackwell-specific kernel/shape bug.
