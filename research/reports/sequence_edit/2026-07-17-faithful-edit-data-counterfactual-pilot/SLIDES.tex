...
\begin{frame}{Direct step attention also fails the causal gates}
  \centering
  \begin{tabular}{lr}
    \toprule
    ...
    Shuffled/matched error $\geq 1.05$ & 0.99933 \\
  ...
  \vfill
  \begin{itemize}
    \item Direct observed-step access creates some local separation, but not reliable action use.
    \item The recursive score is unassessed: this branch has no rollout API, and the dormant core is not a substitute.
    \item No coefficient, K/data, hierarchy, rollout, or LDAD continuation is justified.
  \end{itemize}
  \textbf{Decision:} retire this state family; require a token-aligned recursive CPU fixture before any restart.
\end{frame}
\end{document}
