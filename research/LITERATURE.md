...
## 2026-07-23 — original iGSM-medium length-generalization evaluation

- Decision: which exact problem-length cells and data split should be used for
  the first generation evaluation of the 124M sequence-edit models.
- Primary sources:
  - Physics of Language Models, Part 2.1, Section 2.4:
    <https://arxiv.org/abs/2407.20311>
  - Official vendored iGSM generator and hash-bin constants:
    `third_party/iGSM/const/params.py`.
- Applicable claims:
  - iGSM-medium trains on `op <= 15` with at most 20 instance parameters. It
    reports mixed `op <= 15` ID, the `op = 15` support boundary, and exact
    length-OOD cells `op in {20,21,22,23}`.
  - The official protocol additionally separates solution-template hashes:
    training bins are 0--15 and test bins are 16--22. Reported paper cells use
    at least 4,096 problems, which is too expensive for an initial
    token-by-token MPC process gate.
- Limitations:
  - The current July-22 edit checkpoints were generated by an adapter that
    passed all 23 hash bins during training. Their fresh exact-`op` evaluation
    can test length generalization, but cannot claim the paper's template-hash
    holdout.
  - These checkpoints trained a content prior but assigned zero loss to GAR
    action value and state-goal-distance heads. Greedy content-prior generation
    is valid; calling deeper lookahead “JEPA MPC” would not be.
- Design change:
  - Run a dependency-held eight-example process gate in the six original
    iGSM-medium length cells, using official test hash bins and explicitly
    marking their training-side hash contamination.
  - Enumerate unresolved token positions and select by content confidence
    rather than learning numeric positions. Score the current checkpoints with
    the learned content prior only. Scale toward 4,096 examples per cell and
    depth ablations only after the process gate finishes and after GAR/value
    heads are trained.
