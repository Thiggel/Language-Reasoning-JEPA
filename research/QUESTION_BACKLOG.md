...
| 1 (sequence edit, active) | Does direct action-conditioned attention over observed buffer steps restore causal edit sensitivity? | Whether to retain this edit-state family or retire it before any scale or hierarchy work. | One K=4, 2k, local-weight-4 cell using the existing attention-over-buffer predictor and the same causal gates. |
