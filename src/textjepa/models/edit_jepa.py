...
class EditJEPA(nn.Module):
    def __init__(
        ...
        if self.proposal_selection not in {"hard", "random", "positive_anchor"}:
            raise ValueError(
    ...
    def select_positive_anchor_candidates(
        ...
        """Select one privileged positive anchor and predicted-hard negatives."""
        if selected_k < 1:
            raise ValueError("selected_k must be positive for adaptive proposals")
        count = min(int(selected_k), scores.shape[-1])
    ...
    def _adaptive_structured_candidates(
        ...
        with torch.no_grad():
            ...
            if self.proposal_selection == "positive_anchor":
                ...
                selected, selected_valid = self.select_positive_anchor_candidates(
                    broad_score, batch["gar_proposal_token_edit_target"],
                    broad_valid, self.selected_k,
                )
            else:
        ...
        if self.proposal_selection == "positive_anchor":
            exact = batch["gar_proposal_token_edit_target"]
            anchor_advantage = exact.gather(-1, selected[..., :1]).squeeze(-1)
            valid_state = broad_valid.any(-1)
            positive_available = ((exact > 0) & broad_valid).any(-1)
            anchor_positive = (anchor_advantage > 0) & selected_valid[..., 0]
            ...
            out.extras.update({
                ...
                "adaptive_positive_anchor_coverage": (
                    (positive_available & valid_state).sum().float() / denominator
                ),
